...
local function list_filter(msg)
	...
	if hash then
		...
		local text = 'List of filter words:\n______________________________\n'
		for i=1, #names do
			text = text..'> '..names[i]..'\n'
		end
		return text
...
local function get_filter_act(msg, var_name)
	...
	if hash then
		local value = redis:hget(hash, var_name)
		if value == 'msg' then
			return 'If you write the word was mentioned'
		elseif value == 'kick' then
			return 'If you write this word removed from the group'
		elseif value == 'none' then
			return 'This is not a word filter'
		end
	end
end
